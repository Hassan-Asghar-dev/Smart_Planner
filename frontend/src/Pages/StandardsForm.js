...
function StandardsForm() {
  ...
  const [educationSystem, setEducationSystem] = useState('NQF');
  ...
  const handleGenerateCurriculum = async () => {
    if (!degree || !subject) {
      setError('Please select both Degree and Subject!');
      return;
    }
    ...
    try {
      const response = await fetch('/api/generate-standard-curriculum/', {
        method: 'POST',
        headers: {
  ...
  const handleDownloadCurriculum = () => {
    if (generatedCurriculum) {
      ...
      link.href = url;
      link.download = `curriculum_${educationSystem}_${degree.replace(/ /g, '_')}_${subject.replace(/ /g, '_')}.txt`;
      ...
      <div className="flex-grow container mx-auto px-6 py-12 flex flex-col lg:flex-row gap-8">
        ...
        <div className="bg-white p-8 rounded-xl shadow-xl w-full lg:w-2/3 transition-all duration-300 hover:shadow-2xl">
          <h2 className="text-3xl font-bold text-indigo-600 mb-6 text-center">
            Generate Curriculum by Standards
          </h2>
          <form onSubmit={handleSubmit} className="space-y-6">
            {/* Education System Select */}
            <div className="relative">
              <label
                htmlFor="educationSystem"
              ...
              <select
                ...
                aria-required="true"
              >
                <option value="NQF">National Qualifications Framework (NQF)</option>
                <option value="HEC">Higher Education Commission (HEC)</option>
                <option value="IPES">International Professional Education Standards (IPES)</option>
              </select>
            </div>
            ...
            <button
              ...
              disabled={!degree || !subject || isGenerating}
        ...
        <div className="bg-white p-8 rounded-xl shadow-xl w-full lg:w-1/3 transition-all duration-300 hover:shadow-2xl">
          ...
          <p className="text-gray-600 mb-6">
            Generate curricula aligned with recognized educational standards like NQF, HEC, or IPES to ensure compliance and quality in academic planning.
          </p>
          <Link
            to="/curriculum"
